...
import { Button, Icon, Input, Row, Col, Table, Drawer, Form, Modal, Upload } from 'antd'
import store from '././../../../stores/store'
import { deleteDisbaled, editDisbaled, listData, createColumn, deleteShow, detailShow, selectedSize, selectedItem, pageSize, currentPage } from '../../../actions/tenant-structure/actionCreator'
import TenantStructureDetail from './TenantStructureDetail'
import columns from './TenantStructureTableConfig'
const ButtonGroup = Button.Group;
const { Search } = Input;
class TenantStructure extends Component {
    constructor(props) {
        super(props);
        //获取state
    ...
    getData = () => {
        ...
        const list = [
            {
                "id": 1, "key": "1", "name": "Han",
                "department": "454362", "description": "Huaxin Aotian Network Technology Co., Ltd.",
                "highestPriviligedRole": "Administrator", "assignedRoles": ["Administrators", "Network Administrators", "Writer", "Reader"]
            },
            {
                "id": 2, "key": "2", "name": "Han-HR",
                "department": "454362", "description": "Human resources of Huaxin Aotian.",
                "highestPriviligedRole": "Network Administrator", "assignedRoles": ["Network Administrators", "Writer", "Reader"]
            },
            {
                "id": 3, "key": "3", "name": "Han-Finance",
                "department": "454362", "description": "Finance Department of Huaxin Aotian.",
                "highestPriviligedRole": "Writer", "assignedRoles": ["Writer", "Reader"]
            },
            {
                "id": 4, "key": "4", "name": "Han-Dev",
                "department": "454362", "description": "R&D Department of Huaxin Aotian.",
                "highestPriviligedRole": "Writer", "assignedRoles": ["Writer", "Reader"]
            },
            {
                "id": 5, "key": "5", "name": "Han-Test",
                "department": "454362", "description": "Testing Department of Huaxin Aotian",
                "highestPriviligedRole": "Reader", "assignedRoles": ["Reader"]
            }
        ]
            ...
            <div>
                <Row>
                    ...
                    <Col span={10} style={{ textAlign: "right" }}>
                        <ButtonGroup>
                            <Button type="primary">
                                <Link to={{ pathname: '/user-center/tenant-structure/mode', state: this.state.selectedItem, mode: "create" }}>
                                    <Icon type="plus"></Icon>
                                </Link>

                            </Button>
                            <Button type="primary" disabled={this.state.editFlag}>
                                <Link to={{ pathname: '/user-center/tenant-structure/mode', state: this.state.selectedItem, mode: "edit" }}>
                                    <Icon type="edit"></Icon>
                                </Link>
                            </Button>
                ...
                <Drawer
                    title="Organization Structure Detail"
                    width={800}
                    closable={false}
                    onClose={this.onClose}
                    visible={this.state.visible}
                >
                    <TenantStructureDetail item={this.state.selectedItem} />

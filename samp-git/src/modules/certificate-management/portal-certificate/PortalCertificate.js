...
import { deleteDisbaled, editDisbaled, listData, createColumn, deleteShow, detailShow, selectedSize, selectedItem, activate, pageSize, currentPage } from '../../../actions/portal-certificate/actionCreator'
import PortalCertificateDetail from './PortalCertificateDetail'
import columns from './PortalCertificateTableConfig'
const ButtonGroup = Button.Group;
const { Search } = Input;
class PortalCertificate extends Component {
    constructor(props) {
        super(props);
        //获取state
        this.state = store.getState().portalCertificateReducer
        console.log("store.getState()", store.getState())
        //订阅redux的状态
        store.subscribe(this.storeChange)
    }
    storeChange = () => {
        this.setState(store.getState().portalCertificateReducer)
    }
    //组件渲染成功后，调用接口去后台请求数据
    componentDidMount() {
        this.getData();
        ...
        const list = [
            {
                ...
                "relateFQDN": "ov2500-upam-cportal.al-enterprise.com"
            },
            {
                "id": 1, "key": "2", "caFileName": "default_ca.pem", "from": "OmniVista-CA",
                "isExpiryStatus": "unexpired", "keyFileName": "default_server.key", "keyPasswd": null, "name": "default_server_1",
                "serverFileName": "default_server.pem", "to": "OmniVista", "type": "Server Certificate",
                "usingStatus": 0, "validityStartTime": 1571280602225, "validityStopTime": 1571280602225,
                "relateFQDN": "ov2500-upam-cportal.al-enterprise.com"
            }
        ...
        const rowSelection = {
            ...
            onSelect: (record, selected, selectedRows) => {
                ...
                if (selectedRows.length === 0) {
                    ...
                    item_name ? store.dispatch(editDisbaled(true)) : store.dispatch(editDisbaled(false))
                } else if (selectedRows.length > 1) {
                    store.dispatch(activate(true))
                    store.dispatch(editDisbaled(true))
            ...
            <div>
                <Row>
                    ...
                    <Col span={10} style={{ textAlign: "right" }}>
                        <Button type="primary" onClick={this.showActivate} style={{ marginRight: "10px" }} disabled={this.state.activateFlag}>
                            Activate
                        </Button>
                        <ButtonGroup>
                            <Button type="primary">
                                <Link to={{ pathname: "/certificate-management/portal-certificate/mode", state: this.state.selectedItem, mode: "create" }}>
                                    <Icon type="plus"></Icon>
                                </Link>
                            </Button>
                            <Button type="primary" disabled={this.state.editFlag}>
                                <Link to={{ pathname: "/certificate-management/portal-certificate/mode", state: this.state.selectedItem, mode: "edit" }} >
                                    <Icon type="edit"></Icon>
                                </Link>
                            </Button>
                ...
                <Drawer
                    title="Captive Portal Certificates Detail"
                    width={600}
                    closable={false}
                    onClose={this.onClose}
                    visible={this.state.visible}
                >
                    <PortalCertificateDetail item={this.state.selectedItem} />
                ...
                <Modal
                    title="Delete Captive Portal Certificates"
...
export default PortalCertificate;
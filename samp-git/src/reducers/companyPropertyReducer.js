import actionTypes from '../actions/company-property/actionTypes';
const defaultState = {
    list: [],
    columns: [],
    editFlag: true,
...
export default (state = defaultState, action) => {
    ...
    if (action.type === actionTypes.IMPORT_SHOW) {
        ...
        newState.showImport = action.value
        return newState
    }
    if (action.type === actionTypes.UPLOADING) {
        let newState = JSON.parse(JSON.stringify(state))
        newState.uploading = action.value
        return newState
    }
    if (action.type === actionTypes.FILE_LIST) {
        let newState = JSON.parse(JSON.stringify(state))
        newState.fileList = action.value
        return newState
    }

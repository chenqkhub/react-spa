import actionTypes from '../actions/self-registration/actionTypes';
const defaultState = {
    list: [],
    copyList: [],
    columns: [],
...
export default (state = defaultState, action) => {
    ...
    if (action.type === actionTypes.COPY_DATA) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.copyList = action.value
        return newState;
    }
    ...
    if (action.type === actionTypes.APPROVE_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.approveFlag = action.value
        return newState;
    }
    if (action.type === actionTypes.REJECT_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.rejectFlag = action.value
        return newState;
    }
    if (action.type === actionTypes.REJECT_ITEM) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.isShowRejected = action.value
        return newState;
    }
    if (action.type === actionTypes.APPROVE_ITEM) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.isShowApproved = action.value
        return newState;
    }

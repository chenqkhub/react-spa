import actionTypes from '../actions/ldap-attribute/actionTypes';
const defaultState = {
    list: [],
    columns: [],
    editFlag: true,
    ...
    fetchModalVisible: false,//fetch模态框
    selectedSize: 0,
    selectedItem: {},
    current: 1,
    pageSize: 5,
...
export default (state = defaultState, action) => {
    ...
    if (action.type === actionTypes.FETCH_SHOW) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.fetchModalVisible = action.value
        return newState;
    }

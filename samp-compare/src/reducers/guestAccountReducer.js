import actionTypes from '../actions/guest-account/actionTypes';
const defaultState = {
    list: [],
    columns: [],
    editFlag: true,
...
export default (state = defaultState, action) => {
    ...
    if (action.type === actionTypes.BACTCH_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.beatchDisabeld = action.value
        return newState;
    }
    ...
    if (action.type === actionTypes.EXTEND_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.extendDisabled = action.value
        return newState;
    }
    if (action.type === actionTypes.EXTEND_SHOW) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.isShowExtend = action.value
        return newState;
    }
    if (action.type === actionTypes.ACCOUNT_TYPE) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.accountType = action.value
        return newState;
    }
    if (action.type === actionTypes.OK_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.okButtonDisabled = action.value
        return newState;
    }

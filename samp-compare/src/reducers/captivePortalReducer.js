import actionTypes from '../actions/captive-portal/actionTypes';
const defaultState = {
    list: [],
    columns: [],
    editFlag: true,
...
export default (state = defaultState, action) => {
    ...
    if (action.type === actionTypes.SHOW_IMPORT) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.isShowImport = action.value
        return newState;
    }
    if (action.type === actionTypes.CUSTOM_DISABLED) {
        let newState = JSON.parse(JSON.stringify(state)) //深度拷贝state
        newState.customFlag = action.value
        return newState;
    }
